...
meanCoef = [1, 0.3, 0.59]';
thetaOriginal  = garchDrawCoefficients3(N, seedCoefficients, meanCoef);


% Set range to be explored. Loop trough the GARCH parameter alpha
interceptRange = 0.7:0.05:1.3;
endOfRange= length(interceptRange);
...
for j=1:endOfRange
   ...
   thetaLoop =    garchChangeCoordinateConstant(interceptRange(j), thetaOriginal);
...
%% Plot 1
figure
interceptRange =interceptRange- meanCoef(1);
subplot(1,3,1)
    plot(interceptRange,varianceSample1./varianceIndividual,'c','LineWidth',2) % optimal
    hold on
    plot(interceptRange,varianceEqual1./varianceIndividual,'--')
    plot(interceptRange, varianceStein1./varianceIndividual, '.-')
    plot(interceptRange, varianceAIC1./varianceIndividual, '.-')
%     plot(lambdaRange, varianceMMA1./varianceIndividual, '.-')
    legend('Plug-In', 'Equal','Stein-like', 'AIC', 'Location', 'northeast')
    title('N=5')
    xlim([min(interceptRange), max(interceptRange)])
    ylim([0, 2])
    xlabel('\eta_1')

subplot(1,3,2)
    plot(interceptRange,varianceSample2./varianceIndividual,'c','LineWidth',2) % optimal
    hold on
    plot(interceptRange,varianceEqual2./varianceIndividual,'--')
    plot(interceptRange, varianceStein2./varianceIndividual, '.-')
    plot(interceptRange, varianceAIC2./varianceIndividual, '.-')
...
subplot(1,3,3)
    plot(interceptRange,varianceSampleAll./varianceIndividual,'c','LineWidth',2) % optimal
    hold on
    plot(interceptRange,varianceEqualAll./varianceIndividual,'--')
    plot(interceptRange, varianceStein./varianceIndividual, '.-')
    plot(interceptRange, varianceAIC./varianceIndividual, '.-')
%     plot(lambdaRange, varianceMMA./varianceIndividual, '.-')
    ...
    ylim([0, 2])
% 
suptitle('Averaging Estimator, \mu(\theta_1) = c_1, ratio of MSE to individual estimator')
