function [errEstFixed, errEstLarge, errEstLargeFavorable, errEstLargeUnfavorable] =...
            uaEstimationErrorInfeasibleWeights(eta, V, d0, i0, Nvalues, trueValue, unitEst)
% uaEstimationErrorInfeasibleWeights Evaluates performance of unit
% averaging with infeasible weights. Arguments: 
% 1. Population heterogeneity parameters eta, units arranged as columns
% 2. Population asymptotic variances V, units arranged along the third
%    dimension
% 3. d0 -- gradient of parameter of interest
...
% 1. errEstFixed -- applying fixed-N regime
...
% 4. errEstLargeUnfavorable -- large-N reegime, units ordered by most bias
    ...
    psiFixed = uaPsiMatrix(eta, V, d0);    % for fixed N mode the Psi matrix can be computed once
    for i=1:Nlen
        ...
        if Ncurrent<i0 % switch to fixed-N mode in case i0 is larger than current sample
            ...
            psiLoop =uaPsiMatrix(eta, V, d0, 1, i0); % it's convenient to construct the Q matrix for every value of
            weightLoop = uaOptimalWeights(psiLoop);     % compute weights
...
%         large-N: favorable bias
        if Ncurrent<i0 % switch to fixed-N mode in case i0 is larger than current sample
            ...
            [~, I] = sort(dists);
            psiLoop =uaPsiMatrix(eta(:, I), V(:, :, I), d0, 1, i0); % it's convenient to construct the Q matrix for every value of
            weightLoop = uaOptimalWeights(psiLoop);     % compute weights
