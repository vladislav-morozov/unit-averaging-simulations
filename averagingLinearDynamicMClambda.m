...
% suptitle('Averaging Estiator, \mu(\theta_1) = E(y_{T+1}|y_T=1=-1, x=1), ratio of MSE to individual estimator')
 ...
 weightsSample2 = zeros(compValue2, endOfRange);
 for j=1:endOfRange
...
%    D = [betaScaled(2, 1)/(1-betaScaled(1, 1))^2; 1/(1-betaScaled(1, 1))]; % long-run
   ...
   % Recreate temporary variance vectors
   weightsSample2InLoop = zeros(compValue2, numReplications);
   ...
   % Inner loop: drawing data
    parfor i=1:numReplications
        ...
        weightsSample2InLoop(:,i)= averagingOptimalWeights(psiMatrixSample(1:compValue2,1:compValue2), nW);
        [j, i]
    end
    
    weightsSample2(:, j) = mean(weightsSample2InLoop,2);
    
 end

 %
 figure
 plot(lambdaRange, weightsSample2)
legend('Unit 1')
xlabel('\lambda_1')
title('Optimal feasible weights, N=25')