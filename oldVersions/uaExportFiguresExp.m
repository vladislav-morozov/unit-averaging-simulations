...
paramApproaches = fieldnames(errorsArray{nID, 1}.(paramName));

thetaExtract=[];
for approachID = 1:length(paramApproaches)
    % Extract name
    approachName = paramApproaches{approachID};
    errorExtract.(approachName) = [];
end

% stack together thetas 
for replID = 1:numReplications
    thetaExtract = [thetaExtract; trueThetaArray{nID, replID}(:, thetaCoord)];
    ...
    for approachID = 1:length(paramApproaches)
       ...
       % Extract errors
       errorApproach = errorsArray{nID, replID}.(paramName).(approachName);
       errorExtract.(approachName) = [errorExtract.(approachName); errorApproach];
    end
end

 

training_data = [thetaExtract, (errorExtract.ind).^2];
training_x = thetaExtract;
training_unrestr = (errorExtract.unrestr).^2;
training_ind = (errorExtract.ind).^2;
%%

f = fit(training_data(:, 1), training_data(:,2),'spline');
plot(f,training_data(:, 1), training_data(:,2))
%% 
load franke
f = fit([x y],z,'lowess');
plot(f,[x y],z)
